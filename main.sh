...
[ "$total_threads" -le 0 ] && { printerr "negative thread count."; exit 3; }

upload_dir="/var/bitspace.me/upload/" # Placeholder folder.
...
while true; do # Infinite loop, Make invocations are throttled to 10Hz.
	...
	[ ! "$?" -eq "0" ] && { printerr "make errored out."; exit 5; }
done